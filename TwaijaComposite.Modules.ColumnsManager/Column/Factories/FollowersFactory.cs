...
{
    public class FollowersFactory:FactoryBase,IColumnPartsFactory
    { 
        public string FactoryName
        {
            get { return TwaijaComposite.Modules.Common.Resources.ColumnTypeKeys.TwitterFollowersKey; }
        }
        ...
        public IRequest CreateRequest(Common.DataInterfaces.IUser user, Dictionary<string, object> parameters)
        {
            ...
            var request=helper.CreateAndConfigureRequest<TwitterProfile_SmallViewmodel, GetFollowersRequest>(user, parameters, m_Container, out selectedFactory);
            if (parameters.ContainsKey(TwaijaComposite.Modules.Common.Resources.CreateColumnEventParameters.TargetScreenNameKey))
            {
                request.SetTargetScreenName(parameters[TwaijaComposite.Modules.Common.Resources.CreateColumnEventParameters.TargetScreenNameKey].ToString());
            }
            else
            {
                request.SetUserId(Convert.ToDecimal(parameters[TwaijaComposite.Modules.Common.Resources.CreateColumnEventParameters.TargetUserIdKey]));
            }
